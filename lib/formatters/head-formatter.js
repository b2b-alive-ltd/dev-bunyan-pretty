...
module.exports = function formatHead(record, config, chalk) {
  const name =
    config.printHost === true
      ? record.module && record.module !== record.name
        ? `${record.hostname}:${record.name}:${record.module}`
        : `${record.hostname}:${record.name}`
      : record.module && record.module !== record.name
      ? `${record.name}:${record.module}`
      : record.name

  const color = config.colorize ? getColor(record, name) : fp.identity
   ...
   * @param {Pretty.LogRecord} record
   */
  function getColor(record, name) {
    const key = record.pid + name + (record.req_id || record.reqId || '')
